...
import s from "../css/detailedAdRevenueChart.module.css";
...
const RevenuePerAdDetailedChart = ({ chartObj, dateRange }) => {
  ...
  function getRevenueByType(networkName, fieldName) {
    const networkData = chartObj.data.data.find(
      (item) => item.networkName === networkName
    );
    if (networkData) {
      const { revenue } = networkData;
      const fieldValue = revenue[fieldName];
...
export default RevenuePerAdDetailedChart;
