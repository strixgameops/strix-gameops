...
import s from "../css/histogramChart.module.css";
...
const HistogramTableChart = ({ chartObj, dateRange }) => {
  ...
  function updateData(newData) {
    if (
      ...
      let dataset = newData.map((dataItem) => parseFloat(dataItem.roi));
    ...
    if (
      ...
      const chart = new Chart(canvasObj, {
        ...
        data: {
          ...
          datasets: [
            {
              data: chartObj.data.data.map((dataItem) => dataItem.roi),
  ...
  if (chartObj.data.data !== undefined) {
    ...
    console.log(
      chartObj.data.data.find(
        (n) => n.networkName === "AdMob".toLowerCase().replace(/\s+/g, "")
      ).roi
    );
  }
...
export default HistogramTableChart;
